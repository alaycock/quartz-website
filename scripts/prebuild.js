...
if (!isCI) {
  ...
  try {
    ...
    if (missingVars.length > 0) {
      ...
      console.log("   Skipping token refresh. Make sure to set these in your .env file or environment.");
    } else {
      // Refresh the token
...
try {
  ...
  if (fs.existsSync(tokenPath)) {
    ...
    const filteredLines = lines.filter(line => !line.startsWith("STRAVA_ACCESS_TOKEN=") && !line.startsWith("STRAVA_BOOTSTRAP_REFRESH_TOKEN="));
