...
export default (() => {
  function ContentMetadata({ cfg, displayClass, fileData, ctx }: QuartzComponentProps) {
    ...
    if (
      !fileData.frontmatter?.tags?.includes("post") &&
      !fileData.frontmatter?.tags?.includes("trip") &&
      !fileData.frontmatter?.tags?.includes("route")
    ) {
      return null
    }
    ...
    if (
      fileData.dates &&
      (fileData.frontmatter?.tags?.includes("post") || fileData.frontmatter?.tags?.includes("trip"))
    ) {
      rowSegments.push(<Date date={getDate(cfg, fileData)!} locale={cfg.locale} />)
    }

    const { route, people, gain, distance, elevation, region, tags, strava } =
      fileData.frontmatter as ArbitraryFrontmatter
    ...
    if (strava) {
      ...
      const stravaHref = stravaValue.startsWith("http")
        ? stravaValue
        : `https://www.strava.com/activities/${stravaValue}`
      statSegments.push(
    ...
    const joinedSegments = rowSegments.flatMap((segment, index) =>
      index === rowSegments.length - 1 ? segment : [segment, " • "],
    )

    return (
      <>
        <div class={classNames(displayClass, "content-meta")}>
          {joinedSegments.length > 0 ? <div class="meta-row">{joinedSegments}</div> : null}
          {statSegments.length > 0 ? <div class="meta-stats">{statSegments}</div> : null}
        </div>
        <hr />
      </>
    )
